...
#define SQLPP_DBLIB_VERSION_SA16        16
#define SQLPP_DBLIB_VERSION_SA17        17
#define SQLPP_DBLIB_VERSION	        17
...
/* The size+1 below does not indicate that the string is null-terminated
 * on a fetch. The extra byte is for the convenience of the application. */
...
/* Structures to use for sqldata areas of various types */
...
/* Used by xxxSIZE macros below */
#define _SQL_OFFSETOF(__typ,__id) ((size_t)&(((__typ*)0)->__id))

/* Number of bytes to allocate for VARCHAR, BINARY, LONGVARCHAR
 * or LONGBINARY structure with n bytes in array */
#define _VARCHARSIZE( n )	( _SQL_OFFSETOF( VARCHAR, array[0] ) + n )
#define _BINARYSIZE( n )	_VARCHARSIZE( n )
